...
class Deck(models.Model):
    ...
    user =  models.ForeignKey(User, on_delete=models.DO_NOTHING,  default=getUserIDorCreate)
    ...
    def getUsersDecks(user_ID):
        decks = Deck.objects.filter(user__id = user_ID)
        return decks

    # returns single deck object
    def getDeck(deck_ID, user_ID):
        try:
            deck = Deck.objects.get(deckId = deck_ID, user__id = user_ID)
...
class Card(models.Model):
    ...
    def getDecksCards(deck_ID, user_ID):
        try:
            cards = Card.objects.filter(deck = deck_ID, user__id = user_ID)
        except ObjectDoesNotExist:
            cards = False
        return cards